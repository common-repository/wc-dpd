...
class DpdParcelShopShippingMethod extends \WC_Shipping_Method
{
    ...
    public function init_form_fields()
    {
        ...
        $this->instance_form_fields = [
            ...
            self::FREE_FIXED_SHIPPING_OPTION_KEY => [
                ...
                'type' => 'price',
                'description' => __('If you exceed the price in the cart, shipping will be free.', 'wc-dpd'),
                'default' => '',
                'desc_tip' => true,
                'placeholder' => wc_format_localized_price(0),
                'class' => 'js-dpd-fixed-shipping-type'
    ...
    {
        $free_shipping = isset($this->instance_settings[self::FREE_FIXED_SHIPPING_OPTION_KEY]) && (float) $this->instance_settings[self::FREE_FIXED_SHIPPING_OPTION_KEY] > 0 ? (float) $this->instance_settings[self::FREE_FIXED_SHIPPING_OPTION_KEY] : 0;
        ...
        $rate = [
            'id' => $this->id,
            'label' => $this->title,
            'cost' => $free_shipping && $cart_total > $free_shipping ? 0 : (float) $this->fee,
            'calc_tax' => 'per_order'
        ];
